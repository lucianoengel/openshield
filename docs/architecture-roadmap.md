...
| SIEM | ~46% | **Alert lifecycle unified + external-log ingest:** `peer_alerts` gains severity/status/dedup_key (6b, D178), webhook HMAC replay-protection (8b, D176), notify dedup (12, D172; durable across restart D207), pruned UEBA baselines (5b, D177), ATT&CK mapping (7, D201). **External-log ingest live (SIEM-4): CEF-over-syslog (D202/D205) + AWS CloudTrail cloud-JSON (D208) both parsed, persisted, and searchable** (`OPENSHIELD_CEF_SYSLOG_LISTEN`, `OPENSHIELD_CLOUDTRAIL_DIR`). Still: no UI, no WEF (Windows XML) ingest yet, no `/logs` HTTP surface yet (the `SearchExternalLogs` method is the query capability), CEF extension columnisation (field-level hunting) a follow-on. |
...
- ✅ **SIEM-4 · External log ingestion (CEF parser D202, syslog listener + external_logs store D205, AWS
  CloudTrail cloud-JSON D208)** — P1 · connector class · M. **WEF (Windows XML) is the remaining format**
  (same connector pattern). Correlating external logs into incidents is the XDR-2/4 lane.
