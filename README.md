...
| 🛡️ **Privacy & data protection** | 🟢 ~85% | The controls a deployment is judged on, built as architecture rather than bolted on: **pseudonymous subjects by default** and purpose tagging on every event; classification that transmits **type, confidence and count** and never the matched content or a reversible hash of low-entropy PII; enforced retention with tombstoning purge that keeps the chain verifiable and **refuses to erase what a legal hold covers**; viewing an investigation is itself a tamper-evident ledger entry; DSAR export; a four-eyes gate before any HR-visible outcome; and a shipped DPIA template. **Exclusions are now configurable and enforced** — personal-folder prefixes and agreed break-time windows, applied *before* classification, so an excluded subject's bytes are never read. | The exclusion's two halves are **not equally complete, and the product says which**: a time window needs only the timestamp and always applies, while a **path** exclusion needs a coverage mode that yields resolved paths — where the subject identity carries none, the exclusion cannot be evaluated, so the event is observed and **counted** (`privacy_exclusions_unevaluable`), because a privacy claim with an unmeasured blind spot is a false statement to a works council. An exclusion suppresses **observation, never an enforcement verdict** — otherwise a break window would be a daily interval in which anything runs, which is a control users could invoke rather than one the operator holds. And an excluded subject is **not protected either**: nothing there is classified, so nothing there is contained. No per-user or per-group exclusions — there is no directory integration, and a pseudonymous subject cannot be mapped to a person at the endpoint. |
...
- 🛡️ **Privacy exclusions, actually wired** — personal-folder prefixes and agreed break-time windows are
  configurable and enforced before classification, so an excluded subject's bytes are never read. The
  predicate had existed and been correct for a long time with **no way to configure it**, while the DPIA
  template asked deployers to record exclusions they could not set. Where a path exclusion cannot be
  evaluated the event is observed and *counted*, so the gap in the claim is a number rather than a
  silence
