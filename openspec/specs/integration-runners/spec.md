# integration-runners Specification

## Purpose
Off-pipeline execution of APPROVED response intents against external systems (SOAR-8, ADR-12 Tier-3).
Unlike every other intent enactment in the platform, these actions are irreversible — intent expiry
restores nothing — so the controls are stronger: a per-connector closed verb set, four-eyes required for
every verb and re-checked by the runner, at-most-once execution, and a durable record linking the intent
id to the API call it caused.
