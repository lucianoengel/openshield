...
The gateway MUST classify the plaintext body of a network request within the gateway process, and
MUST project the result to a content-free classification carrying only detector type, confidence, and
occurrence count — the same boundary-safe shape allowed off-host (D10/D29). The body bytes MUST NOT
appear in the Event, the Decision, the audit row, or anything that crosses the process boundary.

#### Scenario: A request body carrying a CPF is classified without content crossing the boundary
- **WHEN** a network Request whose body contains a valid CPF is processed by the gateway
- **THEN** the resulting classification records the detector type and count with no matched text
- **AND** the Event constructed for the pipeline is a `NetworkSubject` carrying metadata only
