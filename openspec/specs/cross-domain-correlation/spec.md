# cross-domain-correlation Specification

## Purpose
Correlating the entity-keyed, multi-domain alert stream (`unified-alerts`) into incidents an operator
triages (XDR-4). It is the consumer that makes the unified stream worth filling: a distinct-domain window
rule and an ordered domain-sequence rule, both grouped by the XDR graph ENTITY so one asset correlates as
one thing whatever each domain calls it, with severity escalated by domain breadth. Additive to the
single-domain UEBA burst rule, which is unchanged — the two answer different questions and coexist,
distinguished by incident kind.
