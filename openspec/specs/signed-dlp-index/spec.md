# signed-dlp-index Specification

## Purpose
Signing, fail-closed verification, and operator authoring of the k-anonymized DLP detection indexes
(EDM single-value, multi-cell record, IDM document-fingerprint) that ship into the sandboxed worker
(ADR-9). An operator signs an index with an operator key; the node loads it ONLY after the signature
verifies against a trusted operator public key. A compromised distribution path may DISTRIBUTE a signed
index but cannot FORGE the signature, so it cannot inject or poison the detection data the DLP detector
