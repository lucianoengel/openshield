# portable-file-connector Specification

## Purpose
The cross-platform, unprivileged file-observation producer. It is the portable analogue of the Linux fanotify connector (D52): the endpoint engine opens it on operating systems where fanotify does not exist (windows, darwin), so the same agent runs and observes there instead of exiting. Pure-standard-library polling (directory scan + metadata snapshot diff) makes the identical code compile and run on every OS with no privilege and no code signing; it reuses the existing FilesystemSubject Event and FILE_CREATED/FILE_MODIFIED kinds (a new producer, not a core change, D26), emits paths never content (D29), and is observe-only (D1). It is the builder half of PLAT-7 / ADR-11; enforcement on those platforms stays externally gated. Coverage is honest, not adversary-proof: polling misses sub-interval churn and it is non-recursive (the careless-insider design centre).
## Requirements
