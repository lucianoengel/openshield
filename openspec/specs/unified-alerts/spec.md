# unified-alerts Specification

## Purpose
A normalized, entity-keyed alert stream every detection domain writes and one correlation engine reads
(XDR-2) — the foundation of cross-domain correlation. Each alert is bound to the XDR entity (device/
user) it concerns via the entity graph, so alerts from different domains for one asset group by an
entity join; deduplicated by a detector-namespaced key. peer_alerts stays as the UEBA-specific record;
a unified_alerts row is its normalized projection the correlation layer reads.
