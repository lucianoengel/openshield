...
One normalized alert stream every detection domain writes into, keyed to the entity graph so a device or
user can be asked about across domains. Each alert carries a reference to the evidence that produced it,
deduplicates by key, and takes its entity and domain from the ORIGINATING event. Severity is derived
from the closed action set and the title leaks nothing — observables ride the dedup key. The projection
is a derived index, never authoritative.
...
<!-- restored from 2026-07-22-xdr2-unified-alerts -->
...
<!-- restored from 2026-07-26-xdr2-domain-alert-producers -->
