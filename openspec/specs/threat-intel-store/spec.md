# threat-intel-store Specification

## Purpose
The server-side IOC store (SOAR-5): signed threat-intel feeds ingested with verification BEFORE parsing,
stored with feed provenance under snapshot semantics, and materialized back into the SAME matcher the
