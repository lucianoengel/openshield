# cloudtrail-ingest Specification

## Purpose
Parsing AWS CloudTrail JSON deliveries and persisting each event as a searchable external-log record
(SIEM-4), so cloud control-plane activity is queryable beside endpoint and network events. A faithful
