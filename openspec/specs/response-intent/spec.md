# response-intent Specification

## Purpose
The coordinated-response seam (SOAR-7, ADR-12 Tier-2): a signed, versioned, TTL'd intent carrying a verb
from a CLOSED three-word vocabulary, published by the control plane and consumed as verified policy
CONTEXT. The server publishes data; the consumer's local policy decides — an open command channel would let
a compromised control plane express an arbitrary action, which is what the closed vocabulary makes
unexpressible. High-impact verbs require a four-eyes approval bound to the specific intent and a bounded
blast radius; expiry is evaluated on read so a stale containment cannot outlive its issuer.
