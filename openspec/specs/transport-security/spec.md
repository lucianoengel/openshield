...
## Purpose
Channel security for the agent-facing surfaces (enrollment + telemetry): mutual TLS that authenticates both peers and encrypts the wire, layered BENEATH Ed25519 message signing (D50) rather than replacing it — both are enforced. It is opt-in and fail-closed (off by default, no plaintext downgrade, a partial config fails loud), so message attribution and channel confidentiality remain independent guarantees (D55).
## Requirements
