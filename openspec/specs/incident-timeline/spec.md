# incident-timeline Specification

## Purpose
What a correlated incident is MADE of (XDR-5): its contributing alerts in detection order across domains,
each linked to the evidence behind it with an explicit resolution state — `resolved` (found in the audit
ledger, coordinates reported), `unresolved` (a reference whose ledger row is not reachable here, kept
intact rather than hidden), or `derived` (a server-side detection with no originating endpoint event).
The distinction exists because the per-agent forward-secure ledger is a different trust domain from the
fleet aggregate, and a timeline that blurred them would make its strongest-looking claim its most
