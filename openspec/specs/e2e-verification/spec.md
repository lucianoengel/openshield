...
## Purpose
A real container end-to-end test: telemetry published over a real NATS container is verified to land in a real Postgres container, persisted by the actual openshield-server BINARY - proving the built binary, its config and the real wire, not just the in-process Server. One idempotent, self-restoring command (deploy/e2e.sh).
## Requirements
