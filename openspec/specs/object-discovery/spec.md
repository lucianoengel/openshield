...
## Purpose
Finding sensitive data AT REST in an S3-compatible object store (DSPM-1), so the product can answer "where is my sensitive data" and not only "who touched it". A sweep enumerates a bucket, reads a bounded prefix of each object, and feeds the same pipeline every other producer feeds; the connector holds bytes and never parses them. Bounded by construction, and what a sweep did NOT examine is reported — a discovery result is a reassuring absence, and a partial sweep that reads as a complete one is this feature's most expensive failure.
