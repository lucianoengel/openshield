...
bounded (an unterminated/no-newline stream must not grow memory without limit), a session that
stalls between lines MUST be timed out and dropped (no slowloris hold), and the number of concurrent
sessions MUST be capped, with connections beyond the cap refused and counted rather than queued.
