...
# CROSS-VET, NOT JUST CROSS-BUILD. `go build` does NOT compile _test.go files, so a test using a symbol
# that exists only on Linux passes every local check and fails `go vet ./...` on the windows runner —
# which is exactly what happened to a quarantine test using syscall.Mkfifo (D401 -> D406). `go vet`
# type-checks the test files too, and costs about four seconds per platform.
