...
func (s *Server) MetricsHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		metrics := []struct {
			...
			{"openshield_retention_purge_failures_total", "Retention purges that FAILED — data past its window is still there. Distinct from the counter above, which is a purge that ran and could not be written down; without this, a purge failing for months looks exactly like one that was never due (D483).", s.RetentionPurgeFailures.Load()},
			{"openshield_view_audit_failures_total", "Operator reads REFUSED because the view could not be recorded (D483). The refusal is correct — an unrecordable read must not be served — but it takes the whole console read surface down, and /health is exempt from recording, so without this the outage is visible only to the operator staring at a 500.", s.ViewAuditFailures.Load()},
			...
			{"openshield_jsonlog_ingested_total", "JSON-lines log records stored (SIEM-15).", JSONLogIngested.Load()},
			{"openshield_jsonlog_dropped_total", "JSON-lines records that could not be read or stored — a source sending something this parser cannot read at all shows up here and nowhere else.", JSONLogDropped.Load()},
			{"openshield_jsonlog_synthesized_time_total", "JSON-lines records whose timestamp had to be supplied because the document carried none this parser recognises (SIEM-15). THIS ONE MATTERS MORE THAN IT LOOKS: those events are all present, all searchable, and all in the WRONG PLACE on the timeline — a hunt bounded by time misses them while reporting a clean result, and nothing else about that source looks wrong.", JSONLogSynthesizedTime.Load()},
			{"openshield_recurrence_link_failures_total", "Incidents raised without the link to the one they recur from (SOAR-2b) — the incident is intact, but it pages as first-time trouble when it is the fourth return of something a responder already closed.", RecurrenceLinkFailures.Load()},
			{"openshield_escalations_total", "Escalation rungs fired for incidents nobody acknowledged in time (SOAR-9b). A rising number is not a fault in this product — it is the response process missing its own deadlines, which is what the ladder exists to make visible.", EscalationsSent.Load()},
			{"openshield_escalation_failures_total", "Escalation sweeps that errored — a ladder that has stopped climbing looks exactly like a fleet where everything is being acknowledged promptly, and that is the flattering reading.", EscalationFailures.Load()},
		}
