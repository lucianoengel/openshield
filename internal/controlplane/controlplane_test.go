...
func requireDB(t *testing.T) *pgxpool.Pool {
	...
	defer ddlCancel()
	if _, err := pool.Exec(ddlCtx, `DROP TABLE IF EXISTS investigation_views, agent_identities, enrollment_tokens, fleet_telemetry, peer_alerts, audit_entries, key_epochs, anchors, case_notes, cases, legal_holds, incidents, ueba_baselines, external_logs, notify_dedupe, unified_alerts, retention_events, entities, entity_aliases, schema_migrations CASCADE`); err != nil {
