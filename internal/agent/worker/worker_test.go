...
func TestHandleClassifiesInlineContent(t *testing.T) {
	...
	resp := worker.Handle(context.Background(), c, nil, nil, &corev1.ClassifyRequest{
...
func TestHandleNoSubjectIsError(t *testing.T) {
	c := worker.Classifier(classify.New())
	resp := worker.Handle(context.Background(), c, nil, nil, &corev1.ClassifyRequest{RequestId: "r1"})
	if resp.GetError() == "" {
