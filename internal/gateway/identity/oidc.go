...
func (v *OIDCVerifier) Verify(token string) (*Identity, error) {
	...
	if claims.Iss != v.issuer {
		return nil, fmt.Errorf("identity: OIDC token issuer %q != expected %q", claims.Iss, v.issuer)
	}
	if !audienceContains(claims.Aud, v.audience) {
		return nil, fmt.Errorf("identity: OIDC token audience does not include %q", v.audience)
	}
	now := v.now()
	if claims.Exp == 0 || now.After(time.Unix(claims.Exp, 0)) {
		return nil, fmt.Errorf("identity: OIDC token is expired or has no expiry")
	}
	if claims.Nbf != 0 && now.Before(time.Unix(claims.Nbf, 0)) {
		return nil, fmt.Errorf("identity: OIDC token is not yet valid (nbf)")
	}
	if claims.Sub == "" {
		return nil, fmt.Errorf("identity: OIDC token has no subject")
	}
	...
	return &Identity{Subject: pseudonym(claims.Sub), Role: role}, nil
}
