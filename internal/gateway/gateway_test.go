...
// fakeTable stands in for the socket-backed flow table (N1.2). It records which
...
// The network walking skeleton: a request whose BODY carries a CPF flows through
// the REAL policy → decision → audit ledger, landing an ALERT — no sockets, no
// Postgres. This is the network analogue of the endpoint TestWalkingSkeleton.
func TestGatewayWalkingSkeleton(t *testing.T) {
	pol, err := policy.NewDefault(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	led := &recLedger{}
	g := gateway.New(pol, led, nil, time.Second)
	...
	if dec.GetAction() != corev1.Action_ACTION_ALERT {
		t.Fatalf("decision = %v, want ALERT for a request body with a CPF", dec.GetAction())
	}
	if len(led.entries) != 1 {
		t.Fatalf("ledger recorded %d entries, want 1", len(led.entries))
	}
	if led.entries[0].Decision.GetAction() != corev1.Action_ACTION_ALERT {
		t.Errorf("audited decision = %v, want ALERT", led.entries[0].Decision.GetAction())
	}
}

// No body content crosses out of the classify step: the classification the policy
// sees carries type + count with EMPTY matched text (D10/D29). A capturing policy
// stage inspects the State the body-classify stage built.
func TestNoBodyContentCrossesTheBoundary(t *testing.T) {
	...
	g := gateway.New(capture, &recLedger{}, nil, time.Second)

	if _, err := g.Process(context.Background(), req("flow-1", cpfBody)); err != nil {
		t.Fatal(err)
	}
	if captured == nil || len(captured.GetMatches()) == 0 {
		t.Fatalf("expected at least one CPF match from the body, got %v", captured)
	}
...
func TestFlowEnforcerReceivesFlowID(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			tbl := &fakeTable{}
			g := gateway.New(deciding(tc.action), &recLedger{}, nil, time.Second)
...
func TestObserveOnlyDefault(t *testing.T) {
	tbl := &fakeTable{}
	led := &recLedger{}
	g := gateway.New(deciding(corev1.Action_ACTION_BLOCK), led, nil, time.Second)
