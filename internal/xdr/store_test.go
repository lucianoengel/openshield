...
func requireDB(t *testing.T) *pgxpool.Pool {
	...
	defer ddlCancel()
	if _, err := pool.Exec(ddlCtx, `DROP TABLE IF EXISTS entity_aliases, entities, audit_entries, key_epochs, anchors, fleet_telemetry, peer_alerts, agent_identities, enrollment_tokens, investigation_views, case_notes, cases, approvals, legal_holds, incidents, incident_alerts, ueba_baselines, schema_migrations CASCADE`); err != nil {
