...
var EngineFields = []Field{
	...
	{Key: "OPENSHIELD_ENROLL_PREAUTH_TOKEN", Scope: ScopeBootstrap, Kind: KindSecret, Default: "",
		Description: "Operator PRE-AUTHORIZATION for attestation self-enrollment (D317), when the deployment issues them. Empty is correct and common \u2014 a gateway that issues none ignores the field, so one endpoint configuration serves both."},
	{Key: "OPENSHIELD_ATTEST_PCRS", Scope: ScopeBootstrap, Kind: KindString, Default: "",
		Description: "Comma-separated TPM platform-configuration registers the attestation baseline covers (ZT-1), e.g. \"0,7\". Empty disables hardware attestation. An unparseable entry is SKIPPED and announced \u2014 a typo narrows the baseline to something still valid, which no downstream check can catch."},
	{Key: "OPENSHIELD_TPM_ADDR", Scope: ScopeBootstrap, Kind: KindString, Default: "",
		Description: "TPM device or emulator address. Empty selects the platform default."},
	{Key: "OPENSHIELD_ATTEST_SELF_ENROLL", Scope: ScopeBootstrap, Kind: KindString, Default: "",
		Description: "Set to have this endpoint enrol its own attestation key with the gateway. OPT-IN: a device asserting its own identity is exactly what pre-auth tokens and EK anchoring exist to constrain, so enabling it by default would hand that decision to a default."},
	{Key: "OPENSHIELD_ATTEST_INTERVAL", Scope: ScopeBootstrap, Kind: KindDuration, Default: "5m",
		Description: "How often this endpoint re-attests, so the gateway's verdict tracks its CURRENT state and a drift drops it within a cycle."},
