...
var GatewayFields = []Field{
	...
	{Key: "OPENSHIELD_FLEET_CONTROL_SEQ_FILE", Scope: ScopeBootstrap, Kind: KindString, Default: "/var/lib/openshield/gateway-fleet-control.seq",
		Description: "Where the fleet-control REPLAY BOUND is persisted (SEC-B) — the highest control sequence this gateway has applied. Its directory must survive restarts: set it empty and the bound is in memory, so a restart lets every captured control replay until its own expiry. Must not be the same file as OPENSHIELD_SEQ_FILE, and must differ from the engine's on a host running both."},
	...
	{Key: "OPENSHIELD_SOCKS_LISTEN", Scope: ScopeBootstrap, Kind: KindString, Default: "",
		Description: "Address for the ZT-12 SOCKS5 listener, for tooling that does not speak HTTP proxying (ssh ProxyCommand, database clients). Mutual TLS carries the DEVICE credential and an access ticket the USER one; POST /ticket on the access surface mints a ticket. CONNECT only — BIND and UDP ASSOCIATE are refused. Empty disables it, and /ticket then answers 404 rather than handing out credentials for a listener that is not running."},
	{Key: "OPENSHIELD_TICKET_TTL", Scope: ScopeBootstrap, Kind: KindDuration, Default: "10m",
		Description: "How long a SOCKS access ticket stays valid. Not a session lifetime — the tunnel's own re-authorization bounds a session; a ticket only has to survive long enough to open one. Longer means a leaked ticket is worth more."},
