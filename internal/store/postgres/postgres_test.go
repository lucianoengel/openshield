...
func requireDB(t *testing.T) *pgxpool.Pool {
	...
	defer dropCancel()
	if _, err := pool.Exec(dropCtx, `DROP TABLE IF EXISTS investigation_views, agent_identities, enrollment_tokens, audit_entries, key_epochs, anchors, fleet_telemetry, peer_alerts, case_notes, cases, approvals, legal_holds, incidents, incident_alerts, ioc_indicators, ioc_feeds, playbook_steps, playbook_runs, incident_annotations, schema_migrations CASCADE`); err != nil {
...
func TestMigrateIsIdempotent(t *testing.T) {
	...
	// One row per migration FILE (001..033), and no more no matter how many times
	// Migrate runs — that stability is the property under test.
	if n != 33 {
		t.Errorf("schema_migrations rows = %d, want 33 — a migration applied twice "+
